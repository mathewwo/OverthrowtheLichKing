...
pirate::pirate(QWidget *parent): character("images/pirate.png", parent, 3)
{
    move_increment = 1;
    knockback = false;
...
void pirate::moveleft()
{
    int left = rect.x() - move_increment;
    if (knockback){
        left = rect.x() - move_increment*50;
        knockback = false;
    }
...
void pirate::moveright()
{
    int right = rect.x() + move_increment;
    if (knockback){
        right = rect.x() + move_increment*50;
        knockback = false;
    }
...
void pirate::movedown()
{
    int down = rect.y() + move_increment;
    if (knockback){
        down = rect.y() + move_increment*50;
        knockback = false;
    }
...
bool pirate::AI(character *playerobj)
{
...
void pirate::damaged(character *enemy){
